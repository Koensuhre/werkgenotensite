...
function AdminHome() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [users, jobsOpen, jobsDone, cats, reviews, pendingJobs, pendingPros] =
        await Promise.all([
  ...
  const cards = [
    {
      label: "Opdrachten in review",
      value: data?.pendingJobs ?? "—",
      icon: ClipboardCheck,
      href: "/admin/review-opdrachten" as const,
    },
    {
      label: "Professionals in review",
      value: data?.pendingPros ?? "—",
      icon: UserCheck,
      href: "/admin/review-professionals" as const,
    },
