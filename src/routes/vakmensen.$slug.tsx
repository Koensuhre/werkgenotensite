...
export const Route = createFileRoute("/vakmensen/$slug")({
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-4 py-24 text-center">
      ...
      <Link to="/vakmensen" className="mt-4 inline-block text-brand hover:underline">
        ← Terug
      </Link>
    </div>
  ),
    ...
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      ...
      <header className="bg-card-gradient shadow-card mt-6 overflow-hidden rounded-2xl border border-border/60">
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, oklch(0.72 0.18 155 / 0.4), oklch(0.65 0.22 280 / 0.4))",
          }}
        />
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-semibold tracking-tight">
                {pro.company ?? pro.display_name}
              </h1>
              {pro.verified && (
                <span className="inline-flex items-center gap-1 rounded-md bg-brand/10 px-2 py-0.5 text-xs text-brand">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
              ...
              {pro.rating_avg != null && (
                <span className="inline-flex items-center gap-1">
                  <Star className="h-3 w-3 fill-brand text-brand" /> {pro.rating_avg} (
                  {pro.review_count} reviews)
                </span>
              )}
              {pro.response_time && (
                <span className="inline-flex items-center gap-1">
                  <Clock className="h-3 w-3" /> Reageert {pro.response_time}
                </span>
              )}
              {pro.years_experience != null && (
                <span className="inline-flex items-center gap-1">
                  <Award className="h-3 w-3" /> {pro.years_experience} jaar ervaring
                </span>
              )}
            </div>
          </div>
          ...
          <section className="bg-card-gradient shadow-card rounded-xl border border-border/60 p-6">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="aspect-square rounded-lg border border-border/60 bg-surface-2/40"
                />
              ))}
            </div>
          </section>
            ...
            <div className="flex items-center justify-between">
              ...
              {pro.rating_avg != null && (
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl font-semibold">{pro.rating_avg}</span>
        ...
        <aside className="space-y-4">
          <div className="bg-card-gradient shadow-card rounded-xl border border-border/60 p-5">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Hoofdregio {pro.city ?? "—"} en omgeving (30 km).
            </p>
          </div>
          <div className="bg-card-gradient shadow-card rounded-xl border border-border/60 p-5">
