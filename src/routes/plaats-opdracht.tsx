...
export const Route = createFileRoute("/plaats-opdracht")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Plaats gratis een opdracht en ontvang binnen 24 uur offertes.",
      },
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 sm:px-6 text-center">
        ...
        <p className="mt-3 text-muted-foreground">
          Je opdracht wordt door een admin gecontroleerd voor hij publiek wordt. Je vindt hem terug
          onder
          <span> </span>
          <a href="/dashboard/projecten" className="text-brand hover:underline">
            Mijn projecten
          </a>
          .
        </p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6">
      ...
      <p className="mt-2 text-muted-foreground">
        Het kost 2 minuten en je betaalt niets. Een admin controleert je opdracht voor publicatie.
      </p>

      <form
        onSubmit={onSubmit}
        className="bg-card-gradient shadow-card mt-8 space-y-5 rounded-2xl border border-border/60 p-6"
      >
        <Field label="Wat is de klus?" id="title">
          <input
            ...
            name="title"
            required
            placeholder="bv. Woonkamer schilderen 35m²"
            className="vk-input"
          />
        </Field>
        ...
        <Field label="Beschrijving" id="desc">
          <textarea
            ...
            required
            rows={5}
            placeholder="Beschrijf de klus, materiaal en wanneer het moet gebeuren..."
            className="vk-input min-h-[140px]"
          />
        </Field>
        ...
        <Field label="Foto's (optioneel)" id="photos">
          <label
            htmlFor="photos"
            className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-border bg-surface/60 px-4 py-8 text-sm text-muted-foreground hover:border-brand/40"
          >
        ...
        <button
          ...
          className="mt-2 w-full rounded-lg bg-brand-gradient px-4 py-3 text-sm font-medium text-brand-foreground shadow-glow disabled:opacity-60"
        >
          {submitting ? "Versturen…" : "Verstuur opdracht ter review"}
        </button>
        {!user && !loading && (
          <p className="text-xs text-muted-foreground text-center">
            Je moet ingelogd zijn om een opdracht te plaatsen.
          </p>
        )}
      </form>
